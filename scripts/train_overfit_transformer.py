...
def main(args=None):
    ...
    parser.add_argument("--model-prefix", type=str, default="transformer_elo_optimized",
                        help="Model prefix for saved models (default: transformer_elo_optimized)")
    ...
    # Callbacks
    # Create comprehensive metrics callback for rich visualizations
    ...
    callbacks = [
        ...
        metrics_callback,  # Comprehensive metrics tracking
        ...
        # CSV logger for easy plotting (basic metrics, kept for compatibility)
    ...
    # Return comprehensive history and metadata for visualization
    comprehensive_metrics = metrics_callback.get_history()
    
    return {
        # Basic Keras history (for compatibility)
        'history': history.history,
        # Comprehensive metrics with Elo-scale values
        'detailed_history': comprehensive_metrics['history'],
        'summary': comprehensive_metrics['summary'],
        'best_metrics': comprehensive_metrics['best_metrics'],
        # Paths for loading saved files
        'log_dir': str(RUN_DIR),
        'metrics_csv': str(RUN_DIR / 'metrics.csv'),
        'detailed_metrics_csv': str(RUN_DIR / 'detailed_metrics.csv'),
        'training_history_json': str(RUN_DIR / 'training_history.json'),
...
def train_with_config(**config):
    ...
    Args:
        **config: Configuration parameters as keyword arguments.
                  See main() function for available parameters.
    ...
    Example:
        train_with_config(
            ...
            lr=1e-4,
            shuffle_buffer=5000,
            model_prefix="transformer_overfit_test",
            no_parquet=False,
            no_mixed_precision=True,
            num_gpus=4,
            skip_gpu_init=True,
            max_train_samples=1000,  # Optional: limit samples
            max_val_samples=500,
        )
    """
